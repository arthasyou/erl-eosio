...
-export([characters_to_list/1, characters_to_binary/1, to_2_float/1, to_4_float/1, binary_to_hex/1]).

-export([json_to_list/1]).
...
to_2_float(Float) when is_float(Float) ->
    list_to_float(hd(io_lib:format("~.2f",[Float])));
to_2_float(Float) -> Float.

to_4_float(Float) when is_float(Float) ->
    list_to_float(hd(io_lib:format("~.4f",[Float])));
to_4_float(Float) -> Float.
...
binary_to_hex(B) ->
    Bin = list_to_binary([io_lib:format("~2.16.0b", [N]) || N <- binary_to_list(B)]),
    binary_to_list(Bin).


json_to_list(Data) ->
    AA = lists2:to_json(Data),
    BB = rfc4627:encode(AA),
    Bin = list_to_binary([io_lib:format("~2.16.0b", [N]) || N <- BB]), %% 转16进制不足2位用0补齐
    binary_to_list(Bin).
